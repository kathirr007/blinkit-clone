  ...
  <div class="max-w-4xl mx-auto px-4 py-4">
    ...
    <div class="flex items-center gap-3 bg-white rounded-lg border border-gray-200 px-4 py-3 mb-4 shadow-sm">
      ...
      <input
        ...
        @input="onSearch"
      />
      <button v-if="query" @click="clearSearch" class="p-1 hover:bg-gray-100 rounded">
    ...
    <div v-if="!query && recentSearches.length > 0" class="mb-6">
      <div class="flex items-center justify-between mb-2">
        ...
        <button @click="clearRecent" class="text-xs text-gray-400 hover:text-gray-600">Clear all</button>
      </div>
    ...
    <div v-if="query && searchResults.length > 0">
      <p class="text-sm text-gray-500 mb-3">{{ searchResults.length }} results for "{{ query }}"</p>
    ...
    <div v-else-if="!query && recentSearches.length === 0" class="text-center py-12">
      <Icon name="mdi:magnify" class="w-16 h-16 text-gray-300 mx-auto mb-4" />
      <p class="text-gray-500">Search for groceries, fruits, snacks and more</p>
    </div>
  </div>
</template>
