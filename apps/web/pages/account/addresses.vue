<template>
  <div class="max-w-2xl mx-auto px-4 py-4">
    ...
    <div v-if="addresses.length === 0" class="text-center py-16">
      ...
      <h3 class="text-lg font-medium text-gray-700 mb-2">No saved addresses</h3>
      <p class="text-gray-500 mb-6">Add an address for faster checkout</p>
      <button class="btn-primary" @click="showAddModal = true">Add Address</button>
    </div>
        ...
        <div class="flex items-start justify-between">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600 mt-0.5">{{ address.fullAddress }}</p>
              <p v-if="address.landmark" class="text-xs text-gray-400 mt-0.5">Landmark: {{ address.landmark }}</p>
            </div>
          </div>
          <div class="flex items-center gap-1">
            <button @click="editAddress(address)" class="p-2 hover:bg-gray-100 rounded-full transition-colors">
              <Icon name="mdi:pencil" class="w-4 h-4 text-gray-500" />
            </button>
            <button @click="deleteAddress(address.id)" class="p-2 hover:bg-red-50 rounded-full transition-colors">
    ...
    <Modal v-model="showAddModal">
      <template #title>{{ editingId ? 'Edit Address' : 'Add New Address' }}</template>
      <div class="space-y-3">
        <div>
        ...
        <div>
          ...
          <textarea v-model="addressForm.fullAddress" class="input-field" rows="3" placeholder="House/Flat No., Street, Area"></textarea>
        </div>
        <div>
      ...
      <template #footer>
        <div class="flex gap-3">
          ...
          <button class="btn-primary flex-1" @click="saveAddress">{{ editingId ? 'Update' : 'Save' }}</button>
        </div>
      </template>
    </Modal>
  </div>
